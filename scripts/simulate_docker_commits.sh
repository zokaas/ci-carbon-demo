...
SIM_B_START_ISO=$(date -u -d "${START_TIME}" '+%Y-%m-%dT%H:%M:%SZ' 2>/dev/null || date -u -j -f '%Y-%m-%d %H:%M:%S' "${START_TIME}" '+%Y-%m-%dT%H:%M:%SZ')
echo "  SIM_B_START=${SIM_B_START_ISO}"
echo ""
echo "  WAIT 10 minutes after this script ends, then verify in GMT dashboard"
echo "  that all 160 runs have arrived. Use that moment as SIM_B_END:"
echo "    SIM_B_END=\$(date -u '+%Y-%m-%dT%H:%M:%SZ')"
echo ""
echo "Check data at:"
